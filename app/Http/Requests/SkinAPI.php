...
class SkinAPI
{
    public static function getConstraintsSkins()
    {
        $width = (int) config('skin.width', 64);
        $height = (int) config('skin.height', 64);
        $scale = (int) config('skin.scale', 1);
    ...
    public static function getConstraintsCapes()
    {
        $width = (int) config('skin.width', 64);
        $height = (int) config('skin.height', 32);
        $scale = (int) config('skin.scale', 1);
    ...
    public static function getRule($whats)
    {
        $result = '';

        switch($whats){
            case 'skin':
                foreach (self::getConstraintsSkins() as $key => $value) 
                    $result .= "{$key}={$value},";
                break;
            case 'cape':
                foreach (self::getConstraintsCapes() as $key => $value) 
                    $result .= "{$key}={$value},";
                break;
            default:
                foreach (self::getConstraintsSkins() as $key => $value) 
